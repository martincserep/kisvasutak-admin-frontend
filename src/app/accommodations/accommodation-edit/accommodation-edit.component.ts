...
export class AccommodationEditComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    fetch(environment.apiUrl + '/getacc/' + this.trainId + '/' + this.accommodationId)
      .then((res) => res.json())
      .then((json) => {
        console.log(json)
        this.accommodation = new Accommodation(json[0].name,json[0].imgUrl,json[0].description,json[0].additionalUrl,json[0].location)
      })
      .then(() => {
        this.isLoaded = true
  ...
  sendAccommodation() {
      this.httpClient.patch(environment.apiUrl + '/accomodations/' + this.trainId, {
        trainId: "-" + this.trainId,
        id: "-" + this.accommodationId,
        accObject: this.accommodation
      }).toPromise().catch((res) => {
        if(res.status === 200) {
          this.modalService.open(this.successElement,{windowClass: 'modal', backdropClass: 'modal-backdrop' , centered: true, size: 'xl'})
        }
      })
  }
