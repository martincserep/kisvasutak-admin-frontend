...
export class TrainEditComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    fetch(environment.apiUrl + '/gettrain/' + this.trainId )
      ...
      .then((json) => {
        ...
        this.train = new Train(json[1].name, json[1].imgUrl, json[1].pricingUrl, json[1].scheduleUrl, json[1].description, json[1].eventsUrl, json[1].actualEventsUrl, json[1].additionalUrl, stations, json[1].users)
      })
      .then(() => {
        this.isLoaded = true
  ...
  sendTrain() {
    this.httpClient.patch(environment.apiUrl + '/trains/' + this.trainId, {
        id: "-" + this.trainId,
  ...
  addStation(){
    if(this.newStationName.length > 2 && this.newStationLat !== null && this.newStationLng !== null ) {
      this.train.stations.push(new Station(this.newStationName, new Location(this.newStationLat,this.newStationLng)))
