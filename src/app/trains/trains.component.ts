    ...
    fetch(environment.apiUrl +'/trains/'+ userId )
      ...
      .then((json) => {
        this.allTrain = json.map(current => {
          ...
          return {id: this.trainId, train: new Train(current.value.name, current.value.imgUrl, current.value.pricingUrl, current.value.scheduleUrl, current.value.description, current.value.eventsUrl, current.value.actualEventsUrl, current.value.additionalUrl, stations, current.value.users)};
        })
      }).then(() => {
